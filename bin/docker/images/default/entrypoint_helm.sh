...
if [ "$NEED_SETUP_CONFIG" = "1" ]; then
    ...
    if [ ! ${#CUSTOM_PACKAGES[*]} = 0 ]; then
        ...
        for i in $(seq 0 $cnt_package); do
            ...
            gosu $MAGENTO_DIR_USER composer require --no-update $package
        done
    fi
    ...
    su -c 'composer require --no-update hipay/hipay-fullservice-sdk-php magento/module-bundle-sample-data magento/module-theme-sample-data magento/module-widget-sample-data magento/module-catalog-sample-data magento/module-cms-sample-data magento/module-tax-sample-data && \
