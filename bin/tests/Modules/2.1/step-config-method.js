        ...
        .then(
            function () {
                this.waitForSelector(
                    x('//a[text()="' + method + '"]'), function success()
                    {
        ...
        .then(
            function () {
                this.waitUntilVisible(
                    'select[name="groups[hipay_' + nameField + '][fields][active][value]"]', function success()
                    {
                        var enable = 'select[name="groups[hipay_' + nameField + '][fields][active][value]"]',
                            ...
                            valueEnable = this.evaluate(
                                function (el) {
                                    return document.querySelector(el).value;
                                }, enable
                            ),
                            valueDebug = this.evaluate(
                                function (el) {
                                    return document.querySelector(el).value;
                                }, debug
                            ),
                            valueTest2 = this.evaluate(
                                function (el) {
                                    return document.querySelector(el).value;
                                }, test2
                            );
                    if (typeof option != 'undefined') {
                        var valueOption0 = this.evaluate(
                            function (el) {
                                return document.querySelector(el).value;
                            }, option[0]
                        );
