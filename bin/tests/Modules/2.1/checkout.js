...
exports.selectItemAndOptions = function selectItemAndOptions(test) {
  casper.then(function () {
    ...
    this.waitForSelector(
      'ol.widget-product-grid>li:first-of-type img',
      function success() {
        this.click('ol.widget-product-grid>li:first-of-type img');
      },
      function fail() {
        var altImg = this.getElementAttribute(
          'ol.widget-product-grid>li:first-of-type img',
          'alt'
        );
        test.assertExists(
    ...
    this.waitForSelector(
      ...
      function fail() {
        test.assertNotExists(
          ...
          'Warning message not present on submitting formular'
        );
  ...
  casper.then(function () {
    ...
    this.waitUntilVisible(
      ...
      function fail() {
        test.assertVisible(
          'input#s_method_flatrate_flatrate',
          "'Shipping Method' formular exists"
        );
      },
      35000
    );
 ...
 * @param test
 */
exports.fillStepPayment = function fillStepPayment(
  test,
  hostedField,
  method_hipay,
  currentBrandCC,
  ...
  casper.then(function () {
    this.echo(
      "Choosing payment method and filling 'Payment Information' formular with " +
        currentBrandCC +
        '...',
      'INFO'
    );
      ...
      this.wait(1000, function () {
        ...
        this.wait(10000, function () {
          if (currentBrandCC == 'visa') {
            fillFormMagentoCreditCard(
              parametersLibHiPay.cardsNumber.visa,
              '600',
              hostedField
            );
          } else if (currentBrandCC == 'cb' || currentBrandCC == 'mastercard') {
            fillFormMagentoCreditCard(
              parametersLibHiPay.cardsNumber.cb,
              '600',
              hostedField
            );
          } else if (currentBrandCC == 'visa_3ds') {
            fillFormMagentoCreditCard(
              parametersLibHiPay.cardsNumber.visa_3ds,
              '600',
              hostedField
            );
          } else if (currentBrandCC == 'maestro') {
            fillFormMagentoCreditCard(
              parametersLibHiPay.cardsNumber.maestro,
              '600',
              hostedField
            );
          }
...
function waitAndSendKeys(selector, keys, options) {
  casper.waitForSelector(selector, function () {
    ...
    var elemInfos = casper.getElementInfo(selector),
      ...
      isValidInput =
        tag === 'input' &&
        (typeof type === 'undefined' || supported.indexOf(type) !== -1),
    ...
    var modifiers = utils.computeModifier(
      (options && options.modifiers) || null,
...
function fillFormMagentoCreditCard(card, cvv, hostedFields) {
  if (hostedFields) {
    casper.withFrame(1, function () {
      ...
      waitAndAssertSelectorHasText(
        'input[name="ccname"]',
        'TEST TEST',
        'Payment Page : Cardholder is ok'
      );
    });

    casper.withFrame(2, function () {
      this.test.assertSelectorExists(
...
function clickPayButton() {
  ...
  casper.click(
    '.payment-method._active .actions-toolbar:not([style="display: none;"])>div>button.checkout'
  );
}
...
exports.choosingPaymentMethod = function (test, method_hipay) {
  casper.then(function () {
    ...
    this.waitUntilVisible(
      '#checkout-step-payment input#' + method_hipay,
      function success() {
        this.click('input#' + method_hipay);
        test.info('Done');
      },
