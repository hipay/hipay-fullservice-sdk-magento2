 ...
 * @param test
 */
exports.logToBackend = function logToBackend(
  baseURL,
  admin_login,
  admin_passwd
) {
  casper.thenOpen(baseURL + '/admin', function () {
    ...
    this.waitForSelector(
      ...
      function success() {
        ...
        this.waitForSelector(
          '#menu-magento-backend-dashboard',
          function success() {
 ...
 * @param moto
 */
exports.fillFormHipayEnterprise = function fillFormHipayEnterprise(
  test,
  credentials,
  moto
) {
  casper.then(function () {
    ...
    if (credentials == 'blabla') {
      this.echo(
        'Editing Credentials' +
          stringMoto +
          ' configuration with bad credentials...',
        'INFO'
      );
    } else {
      this.echo(
        'Reinitializing Credentials' + stringMoto + ' configuration...',
        'INFO'
      );
    }
    ...
    if (moto) {
      this.fillSelectors(
        ...
        {
          'input[name="groups[hipay_credentials_moto][fields][api_username_test][value]"]':
            credentials
        },
        false
      );
      ...
      this.fillSelectors(
        'form#config-edit-form',
        {
          'input[name="groups[hipay_credentials][fields][api_username_test][value]"]':
            credentials
        },
        false
      );
    ...
    this.then(function () {
      this.waitForSelector(
        ...
        function fail() {
          test.fail(
            'Failed to apply HiPay Enterprise credentials configuration on the system'
          );
        },
        20000
      );
 ...
 * @param state
 */
exports.setDeviceFingerprint = function setDeviceFingerprint(
  test,
  state,
  configuration
) {
    ...
    .then(function () {
      ...
      valueFingerprint = this.evaluate(function () {
        return document.querySelector(
          'select[name="groups[configurations][fields][fingerprint_enabled][value]"]'
        ).value;
      });
    ...
    .then(function () {
      if (valueFingerprint != state) {
        this.waitForSelector(
          ...
          function success() {
            this.echo(
              'HiPay Enterprise credentials configuration done',
              'INFO'
            );
          },
          function fail() {
    ...
    this.waitForUrl(
      ...
      function success(response) {
        ...
        test.assertExists(
          '.checkout-onepage-success',
          response.status +
            ' The order has been successfully placed with method ' +
            paymentType +
            ' !'
        );
        order.setId(false);
      ...
      function fail() {
        this.echo(
          "Success payment page doesn't exists. Checking for pending payment page...",
          'WARNING'
        );
        this.waitForUrl(
          /hipay\/checkout\/pending/,
          function success() {
            ...
            test.assertExists(
              '.hipay-checkout-pending',
              'The order has been successfully pended with method ' +
                paymentType +
                ' !'
            );
            order.setId(true);
...
exports.checkNotifMagento = function (test, status) {
  try {
    test.assertExists(
      x(
        '//div[@id="order_history_block"]/ul/li[contains(., "Status code: ' +
          status +
          '")][position()=last()]'
      ),
      'Notification ' + status + ' captured !'
    );
    var operation = casper.fetchText(
      x(
        '//div[@id="order_history_block"]/ul/li[contains(., "Status code: ' +
          status +
          '")][position()=last()]'
...
exports.goToOrderDetails = function (test, orderId) {
  casper.then(function () {
    this.waitForSelector(
      '#menu-magento-sales-sales a span',
      function success() {
        this.echo(
          'Checking status notifications from Magento server...',
          'INFO'
        );
      },
      function fail() {
    ...
    this.waitUntilVisible(
      '.item-sales-order.level-2 a span',
      function () {
        this.click('.item-sales-order.level-2 a span');
      },
      function fail() {
        test.assertExists(
          this.visible('.item-sales-order.level-2 a span'),
          'Configuration menu exists'
        );
      },
      30000
    );
