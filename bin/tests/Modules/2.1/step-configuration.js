...
exports.goingToHiPayMethodsConfiguration =
  function goingToHiPayMethodsConfiguration(test) {
    this.goingToConfiguration(test, 'Payment Methods');
  };

exports.goingToConfiguration = function goingToHiPayMethodsConfiguration(
  test,
  name
) {
  casper
    ...
    .then(function () {
      this.waitUntilVisible(
        '.item-system-config.level-2 a span',
        function () {
          this.click('.item-system-config.level-2 a span');
        },
        function fail() {
          test.assertExists(
            this.visible('.item-system-config.level-2 a span'),
    ...
    .then(function () {
      this.waitForUrl(
        ...
        function fail() {
          test.assertUrlMatch(
            /admin\/system_config/,
            'Configuration admin page exists'
          );
        },
        10000
      );
