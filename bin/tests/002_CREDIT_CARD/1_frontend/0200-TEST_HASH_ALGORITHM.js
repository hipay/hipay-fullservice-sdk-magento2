...
casper.test.begin(
    'Change Hash Algorithm ' + paymentType + ' with ' + currentBrandCC, function (test) {
        phantom.clearCookies();

        casper.setFilter(
            "page.confirm", function (msg) {
        ...
        .then(
            function () {
                ...
                this.waitUntilVisible(
                    'button#hashing_algorithm_button', function success()
                    {
                        var current = this.evaluate(
                            function () {
                    ...
                    if (current != 'SHA512') {
                        ...
                        this.thenClick(
                            'button#hashing_algorithm_button', function () {

                                this.waitForSelector(
                                    'aside.modal-popup.confirm._show', function () {
                        ...
                        this.then(
                            function () {
                                this.waitForSelector(
                                    'button#hashing_algorithm_button', function success()
                                    {
                                        var newHashingAlgo = this.evaluate(
                                            function () {
                                                return document.querySelector('#hipay_hashing_algorithm_hashing_algorithm_test').value;
                                            }
                                        );
