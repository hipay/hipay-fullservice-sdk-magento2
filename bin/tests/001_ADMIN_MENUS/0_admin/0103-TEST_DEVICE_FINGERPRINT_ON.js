...
casper.test.begin('Test Magento With Device Fingerprint', function (test) {
  ...
  casper
    ...
    .then(function () {
      ...
      this.waitForSelector(
        ...
        function success() {
          ...
          test.assert(
            this.exists('input#ioBB') && ioBB != '',
            "'ioBB' field is present and not empty !"
          );
    ...
    .then(function () {
      this.echo(
        'Finding order # ' + order.getId() + ' in order list...',
        'INFO'
      );
      this.waitForUrl(
        /manage/,
        ...
        function success() {
          this.thenClick('a[href="#customer-details"]', function () {
            this.wait(1000, function () {
              var BOioBB = this.fetchText(
                x(
                  '//td[text()="Device Fingerprint"]/following-sibling::td/span'
                )
              ).split('.')[0];
              test.assert(
                BOioBB != '' && BOioBB != 'N/A' && ioBB.indexOf(BOioBB) != -1,
                "'ioBB' is correctly present into transaction details of BackOffice TPP with value :" +
                  BOioBB
              );
            });
          });
