...
define([
  ...
  return Component.extend({
    defaults: {
      ...
      afterPlaceOrderUrl:
        window.checkoutConfig.payment.hiPayFullservice.afterPlaceOrderUrl
          .hipay_bancontact_hosted_fields,
      template: 'HiPay_FullserviceMagento/payment/hipay-hosted-fields-local',
      env: window.checkoutConfig.payment.hipay_bancontact_hosted_fields
        ? window.checkoutConfig.payment.hipay_bancontact_hosted_fields.env
        : 'stage',
      apiUsernameTokenJs: window.checkoutConfig.payment
        .hipay_bancontact_hosted_fields
        ? window.checkoutConfig.payment.hipay_bancontact_hosted_fields
            .apiUsernameTokenJs
        : '',
    ...
    getCode: function () {
      return 'hipay_bancontact_hosted_fields';
    },
    getData: function () {
      var self = this;
